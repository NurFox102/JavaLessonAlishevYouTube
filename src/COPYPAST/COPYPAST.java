...
import METHODS.Methods_illustration;
...
___________________________________________________________________________________________________________
        ...
        Methods_illustration method1 = new Methods_illustration();
        object.age = 25;
        object.name = "Alsu";
        method1.sayHello();
        method1.speak(object);

        Сделал класс Person публичным. К полям добавил значение public. Теперь к полям можно обратиться
        из любого класса, который импортирует пакет ClassAndOBJECT.
        В классе Methods_illustration создал метод speak, который принимает в параметр ОБЪЕКТ класса
        Person. В классе main создал объект класса Person object и передаю его в параметр метода speak.
___________________________________________________________________________________________________________
...
___________________________________________________________________________________________________________
        ...
        per1.age = 30;
        Methods_illustration method1 = new Methods_illustration();
        int printFor = method1.calculateYearsToRetirement(per1);
        System.out.println("До пенсии вам " + printFor + " лет работы");

        В классе Methods_illustration я добавил метод calculateYearsToRetirement. Он принимает в
        паратметры объект класса Person. Через созданный экземпляр per1 класса Person задаю поле age.
___________________________________________________________________________________________________________


___________________________________________________________________________________________________________
        Getters_and_Setters getSet = new Getters_and_Setters();
        PersonForGetter piter = new PersonForGetter();
        getSet.method_Setter(piter,"Harvy", 25);
        getSet.method_Getter(piter);    }
___________________________________________________________________________________________________________*/
